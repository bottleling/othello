...
class BitPlayer:
    ...
    def negamax(self, bitBoards, depth, colorValue, alpha, beta, bestIndex):
        ...
        if self.isBoardFull(bitBoards) or depth == 0: return (colorValue * self.heuristic(bitBoards,myColor,myMoves),bestIndex)
        oppMoves = bu.getIndexesOfTrue(bu.getValidMoves(bitBoards,oppColor))
        if len(myMoves) == 0 :
            if len(oppMoves)==0: return (colorValue * self.heuristic(bitBoards,myColor,myMoves),bestIndex)
            return (-99999, None)

        for corner in (0,7,56,63):
            if corner in myMoves:
                return (colorValue * self.heuristic(bitBoards,myColor,myMoves),corner)
